...
export class Minimatch {
  ...
  preprocess(globParts: string[][]) {
    ...
    if (optimizationLevel >= 2) {
      ...
      globParts = firstPhasePreProcess(globParts, this.options)
      globParts = secondPhasePreProcess(globParts, this.options)
    } else if (optimizationLevel >= 1) {
      // just basic optimizations to remove some .. parts
      globParts = levelOneOptimize(globParts, this.options)
    } else {
